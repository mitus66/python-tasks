...
p = Person()
Ми створюємо новий клас за допомогою оператора class та імені класу. За цим слідує блок виразів, що формують тіло класу. В нашому випадку блок у нас порожній, на що вказує оператор pass. Далі ми створюємо об'єкт-екземпляр класу, записуючи ім'я класу з дужками.
...
Наслідування
Поліформізм
З чотирьох концепцій ми розберемо у завданнях три останні, крім абстракції. Це більш просунута категорія, і створення абстрактних класів ми розглядатимемо в другому блоці навчання. Зараз нагадаю йде перший блок Python. Коротко тільки зауважимо, що:
...
Task 1. Створіть клас Animal. Також створіть екземпляр класу Animal (замість реалізації класу можете використовувати pass) і привласніть змінній animal.
'''
...
'''
Об'єкти можуть зберігати дані у звичайних змінних, які належать об'єкту. Змінні, що належать об'єкту або класу, називають полями. Об'єкти можуть мати функції, які належать класу. Такі функції називають методами класу. Всі разом (поля та методи) заведено називати атрибутами класу.
...
p = Person("Boris", 34)
Метод __init__() - спеціальний метод, який автоматично виконується під час створення кожного нового екземпляра на базі класу Person (конструктор). Ім'я методу починається та закінчується двома символами підкреслення. Причина у тому, що так ми запобігаємо конфліктам імен стандартних методів Python і методів ваших класів.

Коли ми створюємо екземпляр Person, Python викликає метод __init__() з класу Person. Ми передаємо в Person() ім'я "Boris" та вік 34 як аргументи. Значення self буде передано автоматично, явно передавати його не потрібно.

Зверніть увагу, що метод класу greeting не приймає параметрів, проте має параметр self у визначенні функції.

Task 2. Створіть клас Animal. Також створіть екземпляр класу Animal та привласніть змінній animal. Для класу Animal у конструкторі створіть дві властивості: nickname - кличка тварини та weight - вага тварини. Реалізуйте також метод класу say. При реалізації методу можна використати оператор pass, поки що головне - це визначення, а не конкретна реалізація.
'''
...
Тут count належить класу Person і є змінною класу. Змінна name належить об'єкту та є змінною об'єкта, і надається значення за допомогою self. Як бачимо з прикладу, звернення до змінної класу Person виконується як Person.count, і ми не використовуємо звернення self.count. До змінної ж об'єкта name, у всіх методах цього об'єкта, ми звертаємось за допомогою позначення self.name.
...
Task 4. Додамо в клас Animal змінну класу color, значення якої спочатку дорівнює 'white', і метод change_color, який повинен змінювати значення змінної класу color.
...
Task 5. Створіть клас Cat, батьківським класом якого є клас Animal. У класі Cat виконайте перевизначення методу say, щоб він повертав рядок "Meow" для екземплярів класу Cat.
...
'''
Як бачимо, клас, що успадковує від іншого, автоматично отримує всі атрибути та методи першого класу. У наших завданнях клас Cat отримує атрибути nickname та weight при ініціалізації екземпляра від класу Animal. Початковий клас Animal називається батьком, а новий клас Cat - нащадком. Клас Cat успадковує атрибути та методи батька, але при цьому також може визначати власні атрибути та методи.
...
cat = Cat("Simon", 10)
Але що буде якщо нам необхідно для класу Cat ввести нову властивість при створенні екземпляра, як порода breed? І виклик екземпляра класу Cat має виглядати так:
...
Task 6. Створіть клас Dog, батьківським класом якого є клас Animal. У класі Dog виконайте перевизначення методу say, щоб він повертав рядок "Woof" для екземплярів класу Dog.
...
'''
При моделюванні явищ реального світу класи з часом доповнюються дедалі більшою кількістю подробиць. Списки атрибутів і методів збільшуються, і через деякий час класи стають довгими та важко підтримуються в розробці. У такій ситуації частину одного класу можна виділити у вигляді окремого класу. Код великого класу розбивається на менші класи, що працюють у взаємодії один з одним.

Наприклад, при подальшому доопрацюванні класу Cat може виявитися, що в ньому стало занадто багато атрибутів та методів, які стосуються господаря кота. У такому разі все, що відноситься до власника кота, можна перемістити в окремий клас ім'ям Owner. Потім екземпляр Owner стає атрибутом класу Cat:
...
Може здатися, що такий варіант потребує більше додаткової роботи, але тепер господаря кота (клас Owner) можна моделювати з будь-яким ступенем деталізації, без втручання у клас Cat.

Task 7. Для минулого завдання додамо клас Owner — власника собаки. У класу є три атрибути: ім'я — name, вік — age та адреса — address. Також необхідно реалізувати метод info, який повертає словник з ключами 'name', 'age' і 'address', та значення яких дорівнюють відповідним властивостям екземпляра класу.
...
'''
Успадкування — це дуже потужний інструмент. Наслідувати можна не тільки від одного класу, а можна одразу від кількох. Таким чином можна отримувати об'єкти, що поєднують у собі властивості багатьох класів. Тут має виникнути питання, а що буде, якщо кілька класів мають атрибути з однаковим ім'ям?

Для відповіді на це питання треба зрозуміти, як Python шукає атрибути (поля або методи) в об'єктах. Власне, це є MRO (Method Resolution Order). MRO у Python працює наступним чином:
...
В цьому прикладі ми створили батьківський клас Mammal, у якого є метод voice та два дочірніх до нього Dog та Cat. Клас Record приймає на вхід методу record_animal об'єкт animal і викликає у нього метод voice, щоб вивести результат виконання voice у консоль. При цьому є клас Chupakabra, у якого теж є метод voice, та хоч він і не успадковується від Mammal, але об'єкти цього класу так само можна передавати в record_animal. Головне, щоб атрибут називався так само і приймав ті ж аргументи (якщо це метод).
...
Tasl 13. Як ми вже говорили, поліморфізм - це здатність програми вибирати різні реалізації при виклику операцій з однією і тією ж назвою.
